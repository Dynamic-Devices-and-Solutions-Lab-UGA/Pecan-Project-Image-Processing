    ...
    switch lower(varargin{k})
		...
		case 'direction'
			if strismember(varargin{k+1},{'in','out'})
				...
				error('IMCINNBUN: unknown direction option %s',thisarg)
			end
			k = k+2;
		case 'rotation'
			if strismember(varargin{k+1},{'ccw','cw'})
				...
				error('IMCINNBUN: unknown rotation option %s',thisarg)
			end
			k = k+2;
		case 'tailpos'
			if strismember(varargin{k+1},{'nw','sw','se','ne'})
				...
				error('IMCINNBUN: unknown tailpos option %s',thisarg)
			end
			k = k+2;
        otherwise
            error('IMCINNBUN: unknown input parameter name %s',varargin{k})
    end
end
